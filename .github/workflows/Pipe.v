...
`define ALUSLLV 4'b1010 // Define ALU code for SLLV
...
initial begin
    ...
    `RPACK(m[5], 4, 2, 4, 0, `SRAV)
    `RPACK(m[6], 6, 2, 6, 0, `SRAV)
    `IPACK(m[7], `BNE, 6, 1, -2)
    `RPACK(m[8], 3, 4, 3, 0, `SUBU)
    `RPACK(m[9], 3, 2, 1, 0, `SRAV)
    `RPACK(m[10], 2, 3, 1, 0, `ADDU)
    `RPACK(m[11], 2, 3, 1, 0, `SLTU)
    `RPACK(m[12], 3, 5, 1, 0, `AND)
    `RPACK(m[13], 5, 3, 1, 0, `OR)
    `RPACK(m[14], 3, 5, 1, 0, `XOR)
    `RPACK(m[15], 4, 3, 1, 0, `SUBU)
    `IPACK(m[16], `ADDIU, 3, 1, -1)
    `IPACK(m[17], `SLTIU, 5, 1, 12345)
    `IPACK(m[18], `ANDI, 3, 1, 3)
    `IPACK(m[19], `ORI, 3, 1, 3)
    `IPACK(m[20], `XORI, 3, 1, 3)
    `IPACK(m[21], `LUI, 0, 1, 1)
    `IPACK(m[22], `LW, 2, 1, 1023)
    `IPACK(m[23], `SW, 0, 2, 1024)
    `IPACK(m[24], `ADDIU, 6, 6, 1)
